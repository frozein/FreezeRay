...
rurt::RendererPath::RendererPath(std::shared_ptr<const Camera> cam, uint32_t imageW, uint32_t imageH, uint32_t maxDepth, uint32_t samplesPerPixel, bool importanceSampling) :
...
rurt::RendererPath::~RendererPath()
{

}

vec3 rurt::RendererPath::li(const std::shared_ptr<const Scene>& scene, const Ray& ray)
{
	vec3 li = vec3(0.0f);
...
vec3 rurt::RendererPath::trace_path(const std::shared_ptr<const Scene>& scene, const Ray& ray)
{
