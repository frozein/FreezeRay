	...
	, m_widthBlocks((width + FR_IMAGE_BLOCK_SIZE - 1) / FR_IMAGE_BLOCK_SIZE), m_heightBlocks((height + FR_IMAGE_BLOCK_SIZE - 1) / FR_IMAGE_BLOCK_SIZE)
{
	...
	for(uint32_t y = 0; y < m_heightBlocks; y++)
	for(uint32_t x = 0; x < m_widthBlocks ; x++)
		m_blocks[x + m_widthBlocks * y] = std::make_shared<T[]>(FR_IMAGE_BLOCK_SIZE * FR_IMAGE_BLOCK_SIZE);
