...
{
public:
	TextureImage(uint32_t width, uint32_t height, std::shared_ptr<const Tmemory[]> image, TextureRepeatMode repeatMode, T multiplier = T());
	...
	static std::shared_ptr<TextureImage<T, Tmemory>> from_file(const std::string& path, bool hdr, TextureRepeatMode repeatMode, T multiplier = T());
	//creates new texture image sharing memory from the old one, but with a different type/scaleVal
	template<typename Tother>
	static std::shared_ptr<TextureImage<T, Tmemory>> from_texture_image(const std::shared_ptr<TextureImage<Tother, Tmemory>>& image, T multiplier = T());
